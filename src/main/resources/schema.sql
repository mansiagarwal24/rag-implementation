-- Direct schema initialization (no Flyway). Idempotent so it is safe to run against a
-- database where the tables already exist. Spring Boot runs this on startup because
...
-- HNSW cosine index prevents sequential scans on vector search.
